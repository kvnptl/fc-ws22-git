...
class MoveRobotServer(Node):
    def __init__(self):
        ...
        self.action_server = ActionServer(self, MoveRobot, '/move_robot', self.server_callback)
        self.robot_position = 0

    def server_callback(self, goal_handle):
        self.get_logger().info('Robot is currently at {self.robot_position}.')
        self.get_logger().info('Received goal to move {goal_handle.request.destination} steps')
